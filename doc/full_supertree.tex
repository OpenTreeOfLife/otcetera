...
by any branch of $T_{2}$. Then $\sigma$ induces a flow on $T_{2}|_{\mathcal{L}(T_{1})}$
(i.e. $T_{2}$ restricted to $\mathcal{L}(T_{1})$). That is, for
each branch of $T_{2}|_{\mathcal{L}(T_{1})}$, $\sigma$ is either
on the left side or the right side of that branch. An important fact
(not shown here, but not very hard) is that each node of $T_{2}|_{\mathcal{L}(T_{1})}$
has either all branches pointing towards the node under the flow,
or at most one branch pointing away. Therefore, starting from any
node, we may follow the flow from node to node. Since this is a flow
on a tree, we may not have cycles. Therefore, there must be a fixed
point. As second important fact (also not shown here) is that when
we find a node where all branches point towards to the node, then
we may impose the split $\sigma$ by adding a branch diving some of
...
at the node that have no taxa of $T_{1}$. These branches are thus
not partitioned to one side or the other side of the new branch. We
solve this ambiguity by attaching them to the rootward side of $\sigma$.
However, in reality the branch imposed by $\sigma$ constitutes a
connected collection of branches over which these clades of $T_{2}$
may wander. Note that not all taxa in $\mathcal{L}(T_{2})-\mathcal{L}(T_{1})$
may wander across the branch imposed by sigma. Taxa in $\mathcal{L}(T_{2})-\mathcal{L}(T_{1})$
that are separated from the new branch by a branch of $T_{2}$ are
instead partitioned on the same side of $\sigma$ as the branch of
$T_{2}$ that attach behind.
