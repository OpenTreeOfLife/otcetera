...
\REQUIRE $v$ an internal node of a different tree where $v.\texttt{leafLSet} \subseteq x.\texttt{leafLSet}$
\ENSURE If $v$ supports $x$, then add an entry to $\supportingPSSet{x}$
\IF{\textsc{IsSupportedBy}$(x, v)$}
    $\supportingPSSet{x} = \supportingPSSet{x} \cup \{(v.\texttt{desIds}, v.\texttt{leafLSet})\}$$
\ENDIF
\end{algorithmic}
\end{algorithm}
...
\IF{$v.\texttt{desIds} \neq  v.\texttt{leafLSet} \cap x.\texttt{desIds}$}
    \RETURN \FALSE \COMMENT{$x$ is not a common ancestor of $v$ or it contains some of the exclude group of $v$.}
\ENDIF
\IF{$v.\texttt{desIds} = v.\texttt{leafLSet} \cap \parent{x}.\texttt{desIds}$}
...
\FOR{each $c \in \children{x}$}
    \IF{$v.\texttt{desIds} = v.\texttt{desIds} \cap c.\texttt{desIds}$}
        \RETURN \FALSE \COMMENT{$x$ is a common ancestor of $v.\texttt{desIds}$, but not the least inclusive ancestor.}
    \ENDIF
\ENDFOR
\RETURN \TRUE
\end{algorithmic}
