...
bool_str_t TaxonomyDiffMaker::add_synonym(const std::string & name,
                                          OttId ott_id,
...
bool_str_t TaxonomyDiffMaker::delete_synonym(const std::string & name, OttId ott_id) {
    auto & tree = this->get_mutable_tax_tree();
    auto & rt_data = tree.get_data();
    auto target_nd = included_taxon_from_id(ott_id); 
    if (target_nd == nullptr) {
...
void TaxonomyDiffMaker::add_name_to_node_maps(const std::string & name,
...
void TaxonomyDiffMaker::remove_name_to_node_from_maps(const std::string & name,
                                                      const RTRichTaxNode * target_nd) {
    auto & tree = this->get_mutable_tax_tree();
    auto & rt_data = tree.get_data();
    auto hit = rt_data.homonym_to_nodes.find(name);
...
bool_str_t TaxonomyDiffMaker::add_forward(OttId former_id, OttId redirect_to_id) {
    auto & tree = this->get_mutable_tax_tree();
    auto & rt_data = tree.get_data();
...
bool_str_t TaxonomyDiffMaker::delete_forward(OttId former_id, OttId redirect_to_id) {
    auto & tree = this->get_mutable_tax_tree();
    auto & rt_data = tree.get_data();
    throw OTCError() << "delete_forward not implemented";
}

bool_str_t TaxonomyDiffMaker::delete_taxon(OttId ott_id) {
    auto & tree = this->get_mutable_tax_tree();
    auto & rt_data = tree.get_data();
    throw OTCError() << "delete_taxon not implemented";
}

bool_str_t TaxonomyDiffMaker::edit_taxon(OttId oid,
                                         OttId parent_id,
...
bool_str_t TaxonomyDiffMaker::add_new_taxon(OttId oid,
                                            OttId parent_id,
                                            const std::string & name,
                                            const std::string & rank,
                                            const std::string & sourceinfo,
...
void TaxonomyDiffMaker::reg_or_rereg_nd(RTRichTaxNode * nnd,
...
TaxonomyRecord & TaxonomyDiffMaker::get_new_tax_rec(OttId oid,
                                                    OttId parent_id,
                                                    const std::string & name,
                                                    const std::string & rank,
                                                    const std::string & sourceinfo,
...
void TaxonomyDiffMaker::write_version_file_contents(std::ostream & out) const {
    out << version << std::endl;
}

void TaxonomyDiffMaker::write_taxonomy_file_contents(std::ostream & tf) const {
...
void TaxonomyDiffMaker::write_synonyms_file_contents(std::ostream & sf) const {
    const string sep = "\t|\t";
...
void TaxonomyDiffMaker::write_forwards_file_contents(std::ostream & ff) const {
    ff << "id\treplacement\n";
...
void TaxonomyDiffMaker::write(const std::string& newdirname) const{
...
void TaxonomyDiffMaker::write_to_stream(std::ostream & out) const {
    out << "version.txt\n";
